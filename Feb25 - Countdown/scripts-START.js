...
function timer(seconds) {
  ...
  const now = Date.now();
  const then = now + seconds * 1000; //now is new to the browser
...
function displayTimeLeft(seconds) {
  ...
  const display = `${minutes}:${remainderSeconds < 10 ? '0' : '' }${remainderSeconds}`; // ? opterator
  document.title = display; //title tag in HTML
